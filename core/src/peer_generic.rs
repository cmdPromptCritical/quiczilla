...
pub async fn run_peer_generic<CS, CR, FS, FR>(
    mut control_send: CS,
    mut control_recv: CR,
    mut file_send: FS,
    mut file_recv: FR,
...
where
    CS: AsyncWrite + Unpin + Send + 'static,
    CR: AsyncRead + Unpin + Send + 'static,
    FS: AsyncWrite + Unpin + Send + 'static,
    FR: AsyncRead + Unpin + Send + 'static,
{
        ...
        loop {
            tokio::select! {
                ...
                res = control_rx.recv() => {
                    match res {
                        Some(m) => {
                            if send_control_msg(&mut control_send, &m).await.is_err() {
                                break;
                            }
                        }
                        None => break,
    ...
    // 2. Control Keep-Alive Ping Task
    let cancel_ping = cancel.clone();
    let control_tx_ping = control_tx.clone();
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                _ = interval.tick() => {
                    if control_tx_ping.send(String::new()).is_err() {
                        break;
                    }
                }
            }
    ...
    tokio::spawn(async move {
        while let Some((path_str, checksum, resume_offset)) = send_file_trigger_rx.recv().await {
            ...
            // Task 3b: Stream writer consumer
    ...
    tokio::spawn(async move {
            ...
            recv_file_trigger_rx.recv().await
        {
            ...
            // Task 4b: Stream network reader producer
            let mut buffer = vec![0u8; FILE_CHUNK_SIZE];
    ...
    let mut pending_send_path: Option<(String, bool, bool)> = None;
    ...
    loop {
        tokio::select! {
            msg_res = read_control_msg(&mut control_recv) => {
                match msg_res {
                    Ok(Some(msg)) => {
                        if msg.is_empty() {
                            continue;
...
async fn handle_accept_generic(
    save_dir: String,
    checksum: bool,
    resume: bool,
    offered_name: String,
...
async fn send_control_msg<W: AsyncWrite + Unpin>(stream: &mut W, msg: &str) -> Result<()> {
...
async fn read_control_msg<R: AsyncRead + Unpin>(stream: &mut R) -> Result<Option<String>> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_peer_generic_transfer_and_pause_resume() {
        ...
        let src_file_path = temp_dir.join("source.bin");
        let dest_dir = temp_dir.join("dest");
        std::fs::create_dir_all(&dest_dir).unwrap();

        // 1. Create a 4 MiB test file with known deterministic bytes
        let test_data: Vec<u8> = (0..4 * 1024 * 1024).map(|i| (i % 251) as u8).collect();
        {
            let mut f = std::fs::File::create(&src_file_path).unwrap();
            f.write_all(&test_data).unwrap();
        }

        // Set up duplex mock streams for control and data
        let (c_client, c_server) = tokio::io::duplex(256 * 1024);
        let (c_client_r, c_client_w) = tokio::io::split(c_client);
        let (c_server_r, c_server_w) = tokio::io::split(c_server);

        let (f_client, f_server) = tokio::io::duplex(256 * 1024);
        let (f_client_r, f_client_w) = tokio::io::split(f_client);
        let (f_server_r, f_server_w) = tokio::io::split(f_server);
        ...
        let client_handle = tokio::spawn(run_peer_generic(
            c_client_w,
            c_client_r,
            f_client_w,
            f_client_r,
        ...
        let server_handle = tokio::spawn(run_peer_generic(
            c_server_w,
            c_server_r,
            f_server_w,
            f_server_r,
        ...
        let _ = server_event_rx.recv().await;

        // Server accepts files
        server_cmd_tx
            .send(PeerCommand::AcceptFile {
                save_dir: dest_dir.to_string_lossy().to_string(),
                checksum: true,
                resume: false,
            })
            .unwrap();

        // Client initiates send and pauses
        client_cmd_tx
            .send(PeerCommand::SendFile {
                path: src_file_path.to_string_lossy().to_string(),
                checksum: true,
                resume: false,
            })
            .unwrap();
        client_cmd_tx.send(PeerCommand::Pause).unwrap();

        // While paused, let event loop tick
        tokio::time::sleep(Duration::from_millis(100)).await;

        // Resume transfer
        client_cmd_tx.send(PeerCommand::Resume).unwrap();

        // Await client completion
        let mut completed = false;
        while let Some(event) = client_event_rx.recv().await {
            if let PeerEvent::TransferComplete { status } = event {
                assert_eq!(status, FileTransferStatus::Completed);
                completed = true;
                break;
            }
        }
        assert!(completed, "Transfer did not complete successfully");

        // Verify destination file content
        let dest_file_path = dest_dir.join("source.bin");
        assert!(dest_file_path.exists());
        let written_data = std::fs::read(&dest_file_path).unwrap();
    ...
    #[tokio::test]
    async fn test_peer_generic_cold_resumption() {
        let temp_dir = std::env::temp_dir().join(format!(
            "quic_test_resume_{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
        ...
        std::fs::create_dir_all(&temp_dir).unwrap();
        let src_file_path = temp_dir.join("large_archive.bin");
        let dest_dir = temp_dir.join("dest");
        std::fs::create_dir_all(&dest_dir).unwrap();

        // Create a 24 MiB test file (> 20 MiB threshold)
        let total_size = 24 * 1024 * 1024;
        let test_data: Vec<u8> = (0..total_size).map(|i| (i % 253) as u8).collect();
        {
            let mut f = std::fs::File::create(&src_file_path).unwrap();
            f.write_all(&test_data).unwrap();
        }

        // Simulate an existing partial file: 6.5 MiB already written on receiver
        // When aligning to 2 MiB, 6.5 MiB should be truncated to 6 MiB (6,291,456 bytes)
        let part_file_path = get_part_path(&dest_dir.join("large_archive.bin"));
        let partial_bytes = 6 * 1024 * 1024 + 500 * 1024; // 6.5 MiB (unaligned)
        {
            let mut f = std::fs::File::create(&part_file_path).unwrap();
            f.write_all(&test_data[..partial_bytes]).unwrap();
        }

        // Set up duplex mock streams
        let (c_client, c_server) = tokio::io::duplex(256 * 1024);
        let (c_client_r, c_client_w) = tokio::io::split(c_client);
        let (c_server_r, c_server_w) = tokio::io::split(c_server);
        ...
        // Server accepts with resume: true
        server_cmd_tx
            .send(PeerCommand::AcceptFile {
                save_dir: dest_dir.to_string_lossy().to_string(),
                checksum: true,
        ...
        // Client initiates send with resume: true
        client_cmd_tx
            .send(PeerCommand::SendFile {
                path: src_file_path.to_string_lossy().to_string(),
                checksum: true,
        ...
        assert!(completed, "Resumed transfer did not complete");

        // Verify part file was renamed to final file
        let dest_file_path = dest_dir.join("large_archive.bin");
        assert!(dest_file_path.exists(), "Final destination file must exist");
        assert!(
            !part_file_path.exists(),
            ".quic-part file should be renamed upon completion"
        );
        ...
        assert_eq!(
            ...
            "Resumed file must be byte-exact match with source"
        );
    ...
    #[tokio::test]
    async fn test_peer_generic_cold_resumption_mismatch_fallback() {
        let temp_dir = std::env::temp_dir().join(format!(
            "quic_test_mismatch_{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
        ...
        std::fs::create_dir_all(&temp_dir).unwrap();
        let src_file_path = temp_dir.join("mismatch.bin");
        let dest_dir = temp_dir.join("dest");
        std::fs::create_dir_all(&dest_dir).unwrap();

        let total_size = 22 * 1024 * 1024;
        ...
        // Create a corrupt/different partial file (all 0xFF) so fingerprint fails
        let part_file_path = get_part_path(&dest_dir.join("mismatch.bin"));
        {
            let mut f = std::fs::File::create(&part_file_path).unwrap();
            f.write_all(&vec![0xFF; 4 * 1024 * 1024]).unwrap();
        }
        ...
        let dest_file_path = dest_dir.join("mismatch.bin");
        assert!(dest_file_path.exists());
        let written_data = std::fs::read(&dest_file_path).unwrap();
        assert_eq!(written_data.len(), total_size);
        assert_eq!(
            written_data, test_data,
            "Mismatch fallback must have re-transferred whole file cleanly"
        );
